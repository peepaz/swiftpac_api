...
class BarcodeGeneratorPNG extends BarcodeGenerator
{
    ...
    public function getBarcode($code, $type, $widthFactor = 2, $totalHeight = 30, $color = array(0, 0, 0))
    {
        ...
        if (function_exists('imagecreate')) {
            ...
            $pngCodeText = imagecreate($this->getWidthPercent((strlen($code) * 4), $png), 15);
            $colorBackground = imagecolorallocate($png, 255, 255, 255);
            $colorBackgroundCodeText = imagecolorallocate($pngCodeText, 255, 255, 255);
            imagecolortransparent($png, $colorBackground);
//             imagecolortransparent($pngCodeText, $colorBackgroundCodeText);
            $colorForeground = imagecolorallocate($png, $color[0], $color[1], $color[2]);
//             $colorForegroundCodeText = imagecolorallocate($pngCodeText, $color[0], $color[1], $color[2]);
        ...
        if ($imagick && isset($imageMagickObject)) {
        	...
        	imagefill($pngCodeText, 0, 0, $colorBackgroundCodeText);
        	imagestring($pngCodeText, 5, 0, 0, $code, imagecolorallocate($pngCodeText, 0, 0, 0));
        	imagecopy($png, $pngCodeText, 0, 0, 0, 0, $this->getWidthPercent(100, $pngCodeText), $this->getHeightPercent(100, $pngCodeText));
            imagepng($png);
            imagedestroy($png);
            imagedestroy($pngCodeText);
        }
        $image = ob_get_clean();
    ...
    public function getWidthPercent($percent,$im){
    
    	$imgWidth = imagesx($im);
    	$pxPercent = number_format($percent/100,2);
    
    	return $imgWidth * $pxPercent;
    
    }
    
   public function getHeightPercent($percent,$im){
