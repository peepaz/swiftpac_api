...
use lib\SwiftPac\DataTypes\SwiftPacBranch;
...
use lib\CargoTrack\DataTypes as CargoTrackDataTypes;
use lib\SwiftPac\db;
...
use lib\SwiftPac\DataTypes\AddressVerification;
use lib\CargoTrack\CargoTrackApiClient;
use lib\CargoTrack\DataTypes\CargoTrackCreateInvoiceRequest;
use lib\CargoTrack\DataTypes\CreateInvoiceRequest;
use lib\CargoTrack\DataTypes\CargoTrackCurrency;
use lib\CargoTrack\DataTypes\InvoiceEntryRecord;
use lib\SwiftPac\DataTypes\SwiftPacShipmentType;
use lib\SwiftPac\DataTypes\ShipmentResponse;
use lib\CargoTrack\DataTypes\CargoTrackGLAccount;
use lib\CargoTrack\DataTypes\CargoTrackBranch;
use lib\SwiftPac\DataTypes\RateResponse;
use lib\SwiftPac\DataTypes\SwiftPacEditAccount;
use lib\SwiftPac\DataTypes\EditAccountResponse;
use lib\SwiftPac\DataTypes\SwiftPacEditAccountResponse;
use lib\SwiftPac\DataTypes\EditAccountResult;
use lib\SwiftPac\DataTypes\EditAccountRequest;
...
class SwiftPacShippingApi {
	...
	protected $invoiceError = false;
	protected $invoiceErrorDetails;
	...
	public static $adminUsername ="spwebservice";
	public static $adminPassword = "svdAdmin123";
	
	//Actions
	public static $adminLoginAction = "login";
	public static $updateAction = "process";
	
	//Urls
	public static $loginUrl = "http://swiftpac.cargotrack.net/default.asp";
	public static $usersUrl = "http://swiftpac.cargotrack.net/appl2.0/settings/users.asp";
	public static $updateAccountUrl = "http://swiftpac.cargotrack.net/appl2.0/accounts/update.asp";
	public static $updateUserUrl = "http://swiftpac.cargotrack.net/appl2.0/settings/update_user.asp";
	...
	public function __construct() {
		...
		$cargoTrackDataTypes = new CargoTrackDataTypes(); // instantiates CargoTrack datatypes to be used in API. This must be instantiated first before using CargoTrack Datatypes
		...
		if ($respErrorObj != null){
			...
			if ($verificationResults != null){
				...
				if ($respErrorObj != null){
					...
					if (!empty($verificationResults["addressVerification"]))
						$respErrorObj[0]->errorDetails->addressVerification = $verificationResults["addressVerification"];
					$swiftPacShipmentResponse->shipmentResponse->errors = $respErrorObj;
					return $swiftPacShipmentResponse;
				}
			}
...
// 				
			...
			if (isset($service)){
				...
				$creditCardPaymentRequest->chargeAmount =  $service->shipmentCharges->rate;
				$creditCardPaymentRequest->currency = $this->shipment->paymentDetail->paymentCurrency;
				$creditCardPaymentRequest->creditCard = $this->shipment->paymentDetail->paymentOption->creditCard;
...
// 				$payment = true;
				...
				$invoiceResult = $this->createCargoTrackInvoice( $service);
				
				if (!$invoiceResult["isCreated"] || !empty($invoiceResult["errorMsg"])){
					
					$this->invoiceError = true;
					$this->invoiceErrorDetails = $invoiceResult["errorMsg"];
					$respErrorObj = $this->returnErrorResponse($swiftPacShipmentResponse->shipmentResponse->errors);
					if ($respErrorObj != null){
						$swiftPacShipmentResponse->shipmentResponse->errors = $respErrorObj;
						return $swiftPacShipmentResponse;
...
// 			$this->var_error_log(array("search"=>$swiftPacAccountSearchResponse));
		...
		if (!empty($swiftPacAccounts)){
			...
			$error = new Error();
			$errorDetails = new ErrorDetails();
			$errorDetails->message = "Account not found";
			$errorDetails->type = "account_search_respond";
			$error->errorDetails = $errorDetails;
...
// 		$this->var_error_log(array("valparams" => $params ));
		...
		$respErrorObj = $this->returnErrorResponse($accountValidationResponse->errors);
		if ($respErrorObj != null){
			$accountValidationResponse->errors = $respErrorObj[0];
...
// 		$this->var_error_log(array("cardpaymentRequest" => $params));
	...
	/**
	 * 
	 * @param SwiftPacEditAccount $params
	 */
	public function SwiftPacEditAccount ($params){
		
// 		$this->var_error_log(array("editAccRq" => $params));
		
		$swiftPacEditAccountRespond = new SwiftPacEditAccountResponse();
		$editAccountResponse = new EditAccountResponse();
		$editAccountResult = new EditAccountResult();
		
		
		$respErrorObj = $this->returnErrorResponse($editAccountResponse->errors);
		if ($respErrorObj != null){
			$editAccountResponse->errors = $respErrorObj[0];
			$swiftPacEditAccountRespond->editAccountResponse  = $editAccountResponse;
		
			return $swiftPacEditAccountRespond;
		}
		
		//Validate username
		$accountUsername = $params->editAccountRequest->accountUsername;
		$accountSearchRequest = new AccountSearchRequest();
		$accountSearch = new AccountSearch();
		$accountSearch->userLogin = $accountUsername;
		$accountSearchRequest->accountSearch = $accountSearch;
		$account = $this->accountSearchSwiftPac($accountSearchRequest);
		
		
		if (!empty($account)){
			
			$isEdited = $this->editUserAccount($params->editAccountRequest);
			if ($isEdited){
				
				$editAccountResult->isEdited = true;
				$editAccountResult->username = $account[0]->user_login;
				$editAccountResponse->addEditAccountResult($editAccountResult);
			}
			else {
				
				$error = new Error();
				$errorDetails = new ErrorDetails();
				$errorDetails->message = "There was an error updating account";
				$errorDetails->type = "703";
				$error->errorDetails = $errorDetails;
				$editAccountResponse->errors = $error;
			}
			
		
		}
		else { //username supplied not found
			$error = new Error();
			$errorDetails = new ErrorDetails();
			$errorDetails->message = "Account not found";
			$errorDetails->type = "702";
			$error->errorDetails = $errorDetails;
			$editAccountResponse->errors = $error;
		}
	...
	/**
	 * 
	 * @param EditAccountRequest $editAccountRequest
	 * @return boolean
	 */
	private function editUserAccount ($editAccountRequest){
		$editAccountList = $editAccountRequest->editAccountList;
		$accountUsername = $editAccountRequest->accountUsername;
		return $this->dbConn->editUserAccount($editAccountList,$accountUsername );
...
// 		$this->var_error_log(array("email" => $cardHolderAddress));
...
// 		$this->var_error_log(array("stripeCustEmail" => $createResult["email"]));
	...
	private function verifyAccount($address,$addresType){
		...
		$addressVerification->type = $addresType;
		$addressError = false;
...
// 		
			if ($email != $accountSearchResult->email){
				...
				$createAccount->branch = $this->getBranch($address);
	...
	// 			if (is_array($createAccountResult)) $createAccountResult = $createAccountResult[0];
				...
				if (isset($createAccountResult)){
					...
					return array ("addressCreationError" => $createAccountResponse->createAccountResponse->createAccountResultList->errors);
				}
				
				if ($addresType == "ShipFrom"){
					
					$this->shipment->shipperAccNum = $accountSearchResult->cargoTrackId;
				}
				else {
					$this->shipment->recipientAccNum = $accountSearchResult->cargoTrackId;
				}
				...
				if ($addresType == "shipFrom"){
					...
					$name = $accountSearchResult->firstName . " ". $accountSearchResult->lastName;
					if (strtoupper($this->shipment->shipFrom->name) != 
							strtoupper($name)) {
								$addressVerification->proposedName = strtoupper($name);
								$addressError = true;
							}
					
					if (strtoupper($this->shipment->shipFrom->addressLine1) != 
							strtoupper($accountSearchResult->address1)) {
								$addressVerification->proposedAddressLine1 = strtoupper($accountSearchResult->address1);
								$addressError = true;
								
							}
					if (strtoupper($this->shipment->shipFrom->city) != 
							strtoupper($accountSearchResult->city)) {
								$addressVerification->proposedCity = strtoupper($accountSearchResult->city);
								$addressError = true;
							}
								
					if (strtoupper($this->shipment->shipFrom->state) != 
							strtoupper($accountSearchResult->state)) {
								$addressVerification->proposedState = strtoupper($accountSearchResult->state);
								$addressError = true;
									
							}
					if (strtoupper($this->shipment->shipFrom->zip) != 
							strtoupper($accountSearchResult->zipCode)) {
								$addressVerification->proposedZip = strtoupper($accountSearchResult->zipCode);
								$addressError = true;
							}	
					
					if (strtoupper($this->shipment->shipFrom->countryCode) != 
							strtoupper($accountSearchResult->countryCode)) {
								$addressVerification->proposedCountryCode = strtoupper($accountSearchResult->countryCode);
								$addressError = true;
							}
				}
				else {
					...
					$name = $accountSearchResult->firstName . " ". $accountSearchResult->lastName;
					$this->var_error_log(array("fasdfa"=>$addressVerification));
						
					if (strtoupper($this->shipment->shipTo->name) != strtoupper($name)) {
								$addressVerification->proposedName = strtoupper($name);
								$addressError = true;
							}
					
					if (strtoupper($this->shipment->shipTo->addressLine1) != strtoupper($accountSearchResult->address1)){
								$addressVerification->proposedAddressLine1 = strtoupper($accountSearchResult->address1);
								$addressError = true;
							}
								
					if (strtoupper($this->shipment->shipTo->city) != 
							strtoupper($accountSearchResult->city)){
								$addressVerification->proposedCity = strtoupper($accountSearchResult->city);
								$addressError = true;
							}		
					
					if (strtoupper($this->shipment->shipTo->state) != 
							strtoupper($accountSearchResult->state)) {
								$addressVerification->proposedState = strtoupper($accountSearchResult->state);
								$addressError = true;
					...
					if (strtoupper($this->shipment->shipTo->zip) != 
							strtoupper($accountSearchResult->zipCode)) {
								$addressVerification->proposedZip = strtoupper($accountSearchResult->zipCode);
								$addressError = true;
							}
					
					if (strtoupper($this->shipment->shipTo->countryCode) != 
							strtoupper($accountSearchResult->countryCode)){
								$addressVerification->proposedCountryCode = strtoupper($accountSearchResult->countryCode);
								$addressError = true;
							}
				...
				if ($addressError) return array ("addressVerification" => $addressVerification);
	 ...
	 * @param Address $address
	 */
	protected function getBranch($address){
		...
		switch ($countryCode){
			...
			case "VC":
				return SwiftPacBranch::ST_VINCENT;
			case "LC":
				return SwiftPacBranch::ST_LUCIA;
			case "US":
				switch ($city){
					case "MIAMI":
						return SwiftPacBranch::USA_MIAMI;
					default:
						return SwiftPacBranch::USA_MIAMI;
						
				}
			case "BB":
				return SwiftPacBranch::BARBADOS;
			case "GD":
				return SwiftPacBranch::GRENADA;
			case "DM":
				return SwiftPacBranch::DOMINICA;
			case "TT":
				return SwiftPacBranch::TRINIDAD;
			default:
				null;
		}
	}
	 ...
	 * @return boolean
	 */
	protected function createCargoTrackInvoice ($service){
		

		$wsdl = "http://cargotrackapi.vincyaviation.com/CargoTrackAPI.php?wsdl";
		
		$cargoTrackApiClient = new CargoTrackApiClient($wsdl,array(
				"username" => "test2",
				"password" => "test1234",
		));
		...
		$accountName ="";
		$accountNumbenr ="";
		$tracking = $this->shipment->reference;
		$units ="1";
		$description = "Online Shipment";
		$priceUnit = $service->shipmentCharges->rate;
		
		if ($shipmentType == SwiftPacShipmentType::FREIGHT_COLLECT) {
			$accountName = $this->shipment->shipFrom->name;
			$accountNumber = $this->shipment->shipperAccNum;
					
		}
		else {
			$accountName = $this->shipment->shipTo->name;
			$accountNumber = $this->shipment->recipientAccNum;
		}
		
		$CargoTrackCreateInvoiceRequest = new CargoTrackCreateInvoiceRequest();
		$CreateInvoiceRequest = new CreateInvoiceRequest();
		$CreateInvoiceRequest->accountName = $accountName;
		$CreateInvoiceRequest->accountNum = $accountNumber;
		$CreateInvoiceRequest->currency = CargoTrackCurrency::USD;
		$CreateInvoiceRequest->date = date("Y-m-d");//format is important (eg. 12-23-2015)
		$CreateInvoiceRequest->createdBy = "Swiftpac Support";
		$CreateInvoiceRequest->updatedBy = "SwiftPac Support";
		$CreateInvoiceRequest->reference = $tracking;
		$CreateInvoiceRequest->warehouseNum = "";
		$CreateInvoiceRequest->branch = CargoTrackBranch::ITS;
		
		$entryRecord1 = new InvoiceEntryRecord();
		$entryRecord1->units = $units;
		$entryRecord1->description = $description;
		
		
		$entryRecord1->glAccount = CargoTrackGLAccount::AIR_FREIGHT;
		
		$entryRecord1->priceUnit = $priceUnit;
	$entryRecord1->subtotal = number_format($entryRecord1->units * $entryRecord1->priceUnit,2);
		
		$CreateInvoiceRequest->addEntryRecord($entryRecord1);
		
		$CargoTrackCreateInvoiceRequest->createInvoiceRequest = $CreateInvoiceRequest;
		
		$this->var_error_log(array("reqa" =>$CargoTrackCreateInvoiceRequest));
		$response = $cargoTrackApiClient->CargoTrackCreateInvoice($CargoTrackCreateInvoiceRequest);
		
		$isCreated = $response->createInvoiceResponse->isCreated;
		
		$errorMsg = $response->error->errorDetails->message;
		
		$this->var_error_log(array("invoiceResp" => $response));
		if ($error){
			return array("errorMsg" => $errorMsg );
		}
		return array ("isCreated" => $isCreated);
	...
	protected function returnErrorResponse($respErrorObj){
		...
		if (!$this->isAuthenticatedVal) {
			...
			$errorType = "401";
			$errorMsg = "Unauthorized: Incorrect API username and/or password.";
			$respErrorObj[] = $this->generateError($errorType, $errorMsg);
			return $respErrorObj;
		}
...
// 				return $respErrorObj;
			...
			if (!$this->isVerifiedShipperAccount){
				...
				$errorType = "shipment_request";
				$errorMsg = "There was an error in verifying the shipper's account address";
				$respErrorObj[] = $this->generateError($errorType, $errorMsg);
				return $respErrorObj;
			...
			if (!$this->isVerifiedRecipientAccount){
				...
				$errorType = "shipment_request";
				$errorMsg = "There was an error in verifying the recipient's account address";
				$respErrorObj[] = $this->generateError($errorType, $errorMsg);
				return $respErrorObj;
		...
		if ($this->invoiceError){
			...
			$errorType = "invoice_request";
			$errorMsg = $this->invoiceErrorDetails;
			$respErrorObj[] = $this->generateError($errorType, $errorMsg);
			return $respErrorObj;
...
// 	}
	...
	private function validateAccountCredientials($accountValidationRequest){
		...
		if ($effectiveUrl != self::$loginUrl) return true;
		return false;
	 ...
	 * 
	 */
	public static function cargoTrackAdminLogin(){
		
		$loginPostinfo = "user=".self::$adminUsername."&password=".self::$adminPassword."&action=".self::$adminLoginAction;
		$ch = curl_init();
