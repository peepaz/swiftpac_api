...
use lib\SwiftPac\DataTypes\SwiftpacPremiumAccountType;
...
// 			$this->var_error_log(array("SearchResponVal" =>$searchResponseVal, "HayStack" => $service->shipmentCharges->fees));
			...
			//Add shipment weight
// 			$packageList = $this->shipment->packageList->packages;
// 			if (!is_array($packageList)) $packageList = array($packageList);
// 			$shipmentWeight = 0;
			
// 			foreach ($packageList as $key=> $package){
// 				$shipmentWeight += $package->weight;
// 			}
			
// 			//update weight of shipment object
// 			$this->shipment->weight = $shipmentWeight;
...
// 		$this->var_error_log(array("searchRequest" => $params));
...
// 		$this->var_error_log(array("accounts" => $swiftPacAccounts));
		...
		if (!empty($swiftPacAccounts)){
			...
			foreach ($swiftPacAccounts as $account){
				...
				$accountSearchResult->zipCode = $account->zipCode;
...
// 		$this->var_error_log(array("header" => $headerInfo));
	 ...
	 *
	 * @param Error $responseObj       	
	 * @return mixed (Error, boolean)
	 */
	protected function returnErrorResponse($respErrorObj,$shipmentObj = null) {
		
		//Determines the function who made the call
		$serviceFunctionName = debug_backtrace () [1] ['function'];
		
// 		switch ($serviceFunctionName){
			
// 			case 'SwiftPacShipment':
// 				if 
// 			break;
// 		}
// 		$this->var_error_log(array("shipment object" => $shipmentObj));
		
		if (isset($shipmentObj)){
		...
		if ($this->isServiceCodeAvailable)$this->isValidServiceCodeVal = $this->isValidServiceCode($this->shipment->serviceCode);
		...
		if (! $this->isValidNonceVal && $this->isAvailClientHeaderVal) {
			...
			return $respErrorObj;
		} else if (! $this->isValidUSCityStateVal && isset($shipmentObj)) {
			...
			return $respErrorObj;
		}else if (! $this->isValidNonUSCountryCodeVal && isset($shipmentObj)){
		...
		else if (!$this->isRateAvailable && isset($shipmentObj)){
		...
		else if (!$this->isValidServiceCodeVal && isset($shipmentObj)){
			$error = new Error ();
			$errorDetails = new ErrorDetails ();
		...
		else if ($serviceFunctionName == "SwiftPacShipment" && !$this->isServiceCodeAvailable){
			$error = new Error ();
			$errorDetails = new ErrorDetails ();
			$errorDetails->type = "424";
			$errorDetails->message = "Missing service code in shipemnt request.";
			$error->errorDetails = $errorDetails;
			$respErrorObj[] = $error;
			return $respErrorObj;
