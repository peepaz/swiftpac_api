...
function getAllWarehouse(){
	...
	curl_setopt($ch, CURLOPT_URL, "http://swiftpac.cargotrack.net/appl2.0/client/warehouse.asp");
...
// 	curl_setopt($ch, CURLOPT_URL, "http://swiftpac.cargotrack.net/appl2.0/print/label_whs_courier.asp?id=128618");
	...
	$numOfOffsets = sizeof($table->children()[0]->children()[1]->children()[0]->children());
	
	echo $numOfOffsets;
	$offset = 0;
	for ($i=0; $i<$numOfOffsets; $i++, $offset += 20){
		
		$warehousePackageData = getWarehouseData ( $ch, $resp, $DOM, $htmlStr, $html);
		
		
		if ($numOfOffsets >1){
			
			curl_setopt($ch, CURLOPT_URL, "http://swiftpac.cargotrack.net/appl2.0/client/warehouse.asp?offset=$offset");
			$resp = curl_exec($ch);
...
// 			$tables = $html->find('table');
			...
			$warehousePackageData1 = getWarehouseData ( $ch, $resp, $DOM, $htmlStr, $html);
			unset($warehousePackageData1[0]);
			$warehousePackageData = array_merge($warehousePackageData,$warehousePackageData1);
...
function getWarehouseData($ch, $resp, $DOM, $htmlStr, $html) {
	...
	// 	$base64 = $resp;
		...
		foreach ($table->children() as $tblKey => $tblRow){
			foreach($tblRow->children() as  $tblRowKey => $tblRowData) {
				...
				if ($tblRowKey == 2 && is_numeric($tblRowData)){
